    ...
    const navigate = useNavigate()
      ...
      const fetchData = async () => {
        try {
          ...
          const response = await axios.get(`http://192.168.15.227:5000/onedata?project_name_id=${project_name}`, {
            headers: {
...
useEffect(() => {
  const save = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`http://192.168.15.227:5000/save`, { project_name }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
