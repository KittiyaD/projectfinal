...
const SQlinject = (props) => {
  ...
  const project_name_n = props.name
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://192.168.15.227:5000/onedata?project_name_id=${project_name_id}`, {
          headers: {
      ...
      .filter(item => item !== null);
        ...
        const Command = response.data[15].select_att_ID_command_DATA[0]
          ...
          console.log("response.data[16].owasp_",response.data[16].owasp_)
            setresponsedata2([
              {"SQL Injection": Index},
...
setresponsedata3(responsedata2)
  ...
  // } 
      try {
        await axios.post(`http://192.168.15.227:5000/addIssue`, {urls, EVIDENCE, Risk, Recommendation, OID, project_name_id},
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
...
// } 
    try {
      await axios.post(`http://192.168.15.227:5000/addIssueActive`, {urls, EVIDENCE, Risk, Recommendation, OID, project_name_id,vul_urls,parameter},
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
    ...
    const sendSeverityToAPI = async (vulnerability, newSeverity) => {
      try {
        const response = await axios.put(
          `http://192.168.15.227:5000/updateSeverityURL`,
          {
            project_name_id,
            vulnerability,
            newSeverity,
    ...
    const refreshData = () => {
      ...
      const handleDelete = async (iddelete) => {
        try {
          ...
          if (result.isConfirmed) {
            await axios.delete(`http://192.168.15.227:5000/oneVulsdelete?project_name_id=${project_name_id}&record=${iddelete}`, {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            });
                            ...
                            <Button  style={{marginLeft:"20px"}} onClick={() => handleConfirmButtonClick(OneData[10], updatedSeverities[index] || OneData[12])}>Confirm</Button>
                                ...
                                <>
                                  {item[Object.keys(item)[0]].map((OneData, dataIndex) => (
                                    ...
                                    {/* fixja */}
                                              <td style={{textAlign:"center",color:"red"}}>{OneData[2]}</td></tr>
                                              </tbody>
                                              </table><table><tbody>
                                      ...
                                      items={[
                                        {
                                          ...
                                          children: (
                                            ...
                                            <div className="collapse-content2" style={{ overflow: 'auto' }}>
                                              <table  style={{ width: '1350px' }}>
                                                <thead>
                                                  <tr>
                                                    <th>URL</th>
