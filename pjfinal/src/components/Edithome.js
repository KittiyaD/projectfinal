...
const Edithome = () => {
    ...
    useEffect(() => {
        axios.get(`http://192.168.15.227:5000/onedata?project_name_id=${project_name_id}`,{
            headers:{
    ...
    const Formsummit = () => {
        ...
        axios.put(`http://192.168.15.227:5000/update`, { project_name, authUser, url, description,project_name_id },{
            headers:{
              Authorization:`Bearer ${token}`,
              'Access-Control-Allow-Origin' : '*',
              'Content-Type': 'application/json'
