...
const URLlist = (props) => {
  ...
  const handleDelete = (iddelete) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `http://192.168.15.227:5000/oneurlsdelete?project_name_id=${project_name_id}&record=${iddelete}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const Formsummit = () => {
    ...
    axios
      .post(
        `http://192.168.15.227:5000/addurls`,
  ...
  const refreshData = async () => {
    ...
    if (confirmationResult.isConfirmed) {
      axios
        .delete(
          `http://192.168.15.227:5000/refreshData?project_name_id=${project_name_id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
